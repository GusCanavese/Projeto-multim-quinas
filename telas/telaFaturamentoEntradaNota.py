...
def telaGerarFaturamentoEntradaNota(self, DadosNota, valorNota, cons):
    ...
    self.frameValorTotais = ctk.CTkFrame(
        self.frameTelaGerarFaturamento, fg_color=self.cor)

    valores = ["Forma de pagamento", "Quantidade parcelas", "Valor"]
    opcoesPagamento = ["", "Dinheiro", "Bndes", "Boleto", "Carnê",
                       "Crédito a vista", "Crédito parcelado", "Débito", "Cheque", "Depósito"]
    ...
    self.botaoAdicionarParcela = ctk.CTkButton(self.frameTelaGerarFaturamento, text="Adicionar Parcela",
                                               width=20, corner_radius=0, command=lambda: verificaParcelasPreenchidas(self))

    # Campos de totais
    labelEntradasTotais = ctk.CTkLabel(self.frameValorTotais, text="Total")
    labelEntradasTotais.place(relx=0.6, rely=0.10)
    self.entradasTotais = ctk.CTkEntry(
        self.frameValorTotais, textvariable=self.totaisFormasDePagamento)
    self.entradasTotais.place(relx=0.6, rely=0.20, relwidth=0.3)

    labelDescontoTotal = ctk.CTkLabel(
        self.frameValorTotais, text="Desconto total")
    labelDescontoTotal.place(relx=0.6, rely=0.35)
    self.descontoTotal = ctk.CTkEntry(
        self.frameValorTotais, textvariable=self.descontoTotalVindoDoPedido)
    self.descontoTotal.place(relx=0.6, rely=0.45, relwidth=0.3)

    labelAcrescimoTotal = ctk.CTkLabel(
        self.frameValorTotais, text="Acrescimo total")
    labelAcrescimoTotal.place(relx=0.6, rely=0.60)
    self.acrescimoTotal = ctk.CTkEntry(
        self.frameValorTotais, textvariable=self.acrescimoTotalVindoDoPedido)
    self.acrescimoTotal.place(relx=0.6, rely=0.70, relwidth=0.3)

    labelValorOriginal = ctk.CTkLabel(
        self.frameValorTotais, text="Valor original da fatura")
    labelValorOriginal.place(relx=0.1, rely=0.10)
    self.ValorOriginal = ctk.CTkEntry(
        self.frameValorTotais, textvariable=self.totaisFormasDePagamento)
    self.ValorOriginal.place(relx=0.1, rely=0.20, relwidth=0.3)

    labelNumeroDaFatura = ctk.CTkLabel(
        self.frameValorTotais, text="Número da fatura")
    ...
    labelValorDaNota = ctk.CTkLabel(
        self.frameValorTotais, text="Valor da nota")
    labelValorDaNota.place(relx=0.1, rely=0.60)
    self.ValorDaNota = ctk.CTkEntry(
        self.frameValorTotais, textvariable=valorNota)
    ...
    def adicionaParcela(self):
        ...
        self.combobox = ctk.CTkComboBox(self.frameTelaGerarFaturamento, width=100, values=opcoesPagamento,
                                        corner_radius=0, command=lambda valor: modal(self, self.entradaValor.get()))
        ...
        self.entradaQuantidade = ctk.CTkEntry(
            self.frameTelaGerarFaturamento, width=100, corner_radius=0, textvariable=self.variavelQuantidade)
        self.entradaQuantidade.place(
            relx=0.4, rely=self.yParcelas, relwidth=0.2)
        self.listaEntradaQuantidade.append(self.entradaQuantidade)

        self.entradaValor = ctk.CTkEntry(
            self.frameTelaGerarFaturamento, textvariable=self.valorDoPedidoVariavel, width=100, corner_radius=0)
...
def modal(self, teste):
    self.frame = criaFrameJanela(
        self.frameTelaGerarFaturamento, 0.5, 0.5, 0.6, 0.9, self.corModal)
    ...
    # campos do modal
    labels = ["Valor a pagar", "Qtd Parcelas",
              "Repetição", "Intervalo", "1ª parcela em:"]
    posicaoxb = 0.14
    ...
    for i in enumerate(labels):
        ...
        if i[0] == 1:
            ...
            campo = ctk.CTkComboBox(
                self.frame, width=100, values=opcoes, corner_radius=0)
            ...
            self.campoRepeticao = ctk.CTkComboBox(
                self.frame, width=100, values=opcoes, corner_radius=0)
            ...
            campo = ctk.CTkEntry(self.frame, width=100,
                                 textvariable=variavel, corner_radius=0)
            campo.place(relx=posicaoxb, rely=0.15)
            entradasLista.append(campo)
            posicaoxb += 0.15
    ...
    for labelsModal in enumerate(self.opcoesLabelModal):
        if labelsModal[0] == 0:
            labelModal = ctk.CTkLabel(
                self.frame, width=60, text=labelsModal[1], fg_color=self.cor)
            labelModal.place(relx=self.posicaoXLabelModal+0.028, rely=0.31)
            self.posicaoXLabelModal += 0.15

        if labelsModal[0] != 0:
            labelModal = ctk.CTkLabel(
                self.frame, width=120, text=labelsModal[1], fg_color=self.cor)
            labelModal.place(relx=self.posicaoXLabelModal-0.04, rely=0.31)
            self.posicaoXLabelModal += 0.15

    botaoCalcular = ctk.CTkButton(self.frame, corner_radius=0, width=100, text="Calcular", command=lambda: calculaParcelasFaturamento.calcularParcelasTotais(
        self, entradasLista[1].get(), entradasLista[0].get(), self.campoRepeticao.get()))
    ...
    botaoSalvarEFechar = ctk.CTkButton(
        ...
        command=lambda: [
            ...
            calculaParcelasFaturamento.SalvaAlteracoesFaturamento(
                self, entradasLista[0].get(), entradasLista[1].get(), variaveis),
            self.calcularTotal(),
            self.frame.destroy()
        ]
    )
    ...
    botaoFechar = ctk.CTkButton(self.frame, text="X", width=10,
                                height=10, corner_radius=0, command=lambda: destroyModal(self))
